...
void run()
{
    ...
    for (int j=0; j<NUM_RUNS; j++) {
        ...
        // The first row of the matrix gets copied to result. The cost of the first row is unavoidable.
        for (int x = 0; x < cols; x++){
            result[x] = wall[x];
        }
        
        // This is done because the next swap will put it into src
        dst = result;
        ...
        for (int t = 0; t < rows - 1; t++) {
            ...
            // For every column in the row that is about to be processed
            for (int n = 0; n < cols; n++){
                // Get the nth element 
                min = src[n];

                // Movement between rows can happen diagonally, check the upper neighbours (if possible) and find the minimum
                if (n > 0)          min = MIN(min, src[n-1]);   // Left one
                if (n < cols-1)     min = MIN(min, src[n+1]);   // Right one

                // On the destination add the minimum of the previous possible routes and the cost of the current cell of the row that is being processed
